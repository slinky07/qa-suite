...
3. If the qa-suite skill is installed on this host, read its
   ...
   one representative action per core flow. Stop at the first hard failure.
...
7. **Mutating smoke actions use disposable, clearly-labeled local test data
   only** — never real credentials, tokens, personal data, or production
   endpoints.
